...
function setupEnvironment() {
  ...
  global.displayMesh = new THREE.Mesh(
...
export function resetTextureSizes() {
  parameterValues.canvas.width =
    canvas.clientWidth * parameterValues.canvas.scale;
  parameterValues.canvas.height =
    canvas.clientHeight * parameterValues.canvas.scale;
...
//==============================================================
function update() {
  // If recording and busy capturing, skip this frame to prevent simulation from advancing
  // This ensures we don't drop frames while waiting for the async capture to finish
  if (globals.isRecording && globals.isCapturingFrame) {
  ...
  if (!globals.isPaused) {
    ...
    for (let i = 0; i < globals.pingPongSteps; i++) {
      var nextRenderTargetIndex =
        globals.currentRenderTargetIndex === 0 ? 1 : 0;

      simulationUniforms.previousIterationTexture.value =
        renderTargets[globals.currentRenderTargetIndex].texture;
      renderer.setRenderTarget(renderTargets[nextRenderTargetIndex]);
      renderer.render(scene, camera);
      simulationUniforms.previousIterationTexture.value =
        renderTargets[nextRenderTargetIndex].texture;
      displayUniforms.textureToDisplay.value =
        renderTargets[nextRenderTargetIndex].texture;
      displayUniforms.previousIterationTexture.value =
        renderTargets[globals.currentRenderTargetIndex].texture;
