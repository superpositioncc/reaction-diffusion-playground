...
function setupActions() {
  ...
  // PNG Sequence export section
  actionsFolder.addInput(globals, "totalRecordingFrames", {
    label: "Frames to record",
    min: 1,
    max: 1000,
    step: 1,
  });

  actionsFolder.addInput(globals, "recordingPrefix", {
    label: "Filename prefix",
  });

  actionsFolder.addInput(globals, "restartBeforeRecording", {
  ...
  if (globals.isRecording) {
    actionsFolder.addMonitor(globals, "currentRecordingFrame", {
      label: "Recording",
      format: (v) => `${Math.floor(v)} / ${globals.totalRecordingFrames}`,
    });
  }

  // Record button - disabled during recording
  const recordButton = actionsFolder.addButton({
    title: globals.isRecording
  ...
  if (globals.isRecording) {
    ...
    recordButton.on("click", () => {
      // Restart pattern if toggle is enabled
      if (globals.restartBeforeRecording) {
        drawFirstFrame(currentSeedType);
      }
